...
def cross_entropy_loss(targets: np.ndarray, outputs: np.ndarray) -> float:
    ...
    N = targets.shape[0]
    loss = 1/N * np.sum(-(targets * np.log(outputs) + (1 - targets) * np.log(1 - outputs)))
    return loss
...
class BinaryModel:
    ...
    def backward(self, X: np.ndarray, outputs: np.ndarray, targets: np.ndarray) -> None:
        ...
        self.grad = -(targets - outputs) * X # gradient wrt w
        self.grad = self.grad.mean(axis=0, keepdims=True).T # mean over batch
